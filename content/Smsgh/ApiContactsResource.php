...
# $Id: ApiContactsResource.php 0 1970-01-01 00:00:00Z mkwayisi $
...
class ApiContactsResource {
    ...
    public function create($object) {
        if ($object instanceof ApiContact) {
            ...
            return new ApiContactGroup(ApiHelper::getJson
                            ($this->apiHost, 'POST', $uri . 'groups/', ApiHelper::toJson($object)));
        } else
            throw new Smsgh_ApiException('Bad parameter type');
    }
    ...
    public function update($object) {
        if ($object instanceof ApiContact) {
            ...
            ApiHelper::getData($this->apiHost, 'PUT', $uri . $object->getContactId(), ApiHelper::toJson($object));
        } else if ($object instanceof ApiContactGroup) {
            $uri = "/" . $this->apiHost->getContextPath() . "/contacts/";
            if ($this->apiHost->getContextPath() == "") {
                $uri = '/contacts/';
            ...
            ApiHelper::getData($this->apiHost, 'PUT', $uri . 'groups/' . $object->getGroupId(), ApiHelper::toJson($object));
        } else
            throw new Smsgh_ApiException('Bad parameter type');
    }
    ...
    public function deleteContact($contactId) {
        ...
        ApiHelper::getData($this->apiHost, 'DELETE', $uri . ($contactId + 0));
    }
    ...
    public function deleteGroup($groupId) {
        ...
        if (is_numeric($groupId)) {
            ApiHelper::getData($this->apiHost, 'DELETE', $uri . 'groups/' . ($groupId + 0));
        } else
            throw new Smsgh_ApiException
            ("Parameter value must be of type 'number'");
    }
