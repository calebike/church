...
# $Id: ApiHelper.php 0 1970-01-01 00:00:00Z mkwayisi $
...
class ApiHelper {
    ...
    (SmsghApi $api, $method, $uri, $data = null) {
        $apiRequest = new ApiRequest
                ($api->getHostname(), $api->getPort(), $api->isHttps() ? 'ssl' : 'tcp', $api->getTimeout(), $api->getClientId(), $api->getClientSecret());
        ...
        if ($apiResponse->getStatus() > 199 && $apiResponse->getStatus() < 300)
            ...
            $exception = new Smsgh_ApiException("Request Failed.");
            $exception->setRawBody($apiResponse->getBody())->setHttpStatusCode($apiResponse->getStatus())->setReason($apiResponse->getReason());
            throw $exception;
        }

        throw new Smsgh_ApiException
        (sprintf('Request failed: (%d) %s', $apiResponse->getStatus(), $apiResponse->getReason()));
    }
